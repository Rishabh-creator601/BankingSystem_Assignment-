...
## code for valiation of apartment id from chatgpt 
...
def update_apartment():
    ...
    # check if user exists and update related info .
    for p in profiles:
        if p.UnitID == apt_id:
...
def update_supplementary_item():
    ...
    ## check if user exists and update related info . while displaying supplementary items
    for p in profiles:
        if p.UnitID == apt_id:
           print(f"Hello {p.name} !")
           print(f" found User {p.name} : Apartment : {apt_id} || rate :  ${p.reward_points} || capacity : { p.capacity}.")
    ...
    # Now, update or add single supplementary item      
